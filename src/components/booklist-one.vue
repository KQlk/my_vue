    ...
    <v-row no-gutters style="margin-top:15px" @click="handleDetails(booklist.books[0].bookId)">
      ...
      <v-col cols="8" sm="8">
        <v-row>
          ...
          <div class="rea--text ml-2" style="margin-right:10px">{{booklist.books[0].bookScore}}分</div>
        </v-row>
        <v-row>
          <v-btn text style="margin:0px; padding:0px" >
            <v-icon left>mdi-account-outline</v-icon>
            {{booklist.book[0].authorName}}
          </v-btn>
        </v-row>
        <v-row style="font-size:13px;color:#gray; margin-top:6px" >{{booklist.books[0].introduction}}</v-row>
      </v-col>
    </v-row>
  </div>
</template>
